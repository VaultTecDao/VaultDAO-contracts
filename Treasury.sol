...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, BOTTLECAPS }
    ...
    address public BOTTLECAPS;
    uint public BOTTLECAPSQueue; // Delays change to BOTTLECAPS address
    ...
    /**
        @notice allow approved address to deposit an asset for VAULT
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint VAULT needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( VAULT ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn VAULT for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITEMERC20( VAULT ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( BOTTLECAPS ).balanceOf( msg.sender ); // Can only borrow against BOTTLECAPS held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with VAULT
        @param _amount uint
     */
    function repayDebtWithTEM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITEMERC20( VAULT ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, VAULT, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( VAULT ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( VAULT ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns VAULT valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match VAULT decimals
            value_ = _amount.mul( 10 ** IERC20( VAULT ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.BOTTLECAPS ) { // 9
            BOTTLECAPSQueue = block.number.add( blocksNeededForQueue );
        } else return false;
